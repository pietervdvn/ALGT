...
In this section, a typechecker for STFL is constructed, using the same notation as introduced in the previous part. Just like the relation `→`, which related two expressions, a relation `::` is defined which relates expression to its type. The relation `::` is thus a relation in `expr × type`.
...
With all the rules defined, we can actually type some expressions. This results in a proof[^shortenedProof] of why an expression has a certain type:

[^shortenedProof]: For printing, the proofs of `==` relations are omitted.
