...
In these paragraphs, the term **subtrahend** refers to the element that is _subtracted_, whereas the **** refers to the element that is _subtracted from_. As mnemonic, the minuend will di\emph{min}ish, whereas the subtrahend _subtracts_.
...
This factorization is calculated by first taking the pointwise differences between the sequences[^sameLength], after which the product with the rest of the sequence is taken.

[^sameLength]: This implies both sequences have the same length. If these don't have the same length, the subtraction would equal the minuend.
...
The set represention can be easily and efficiently transformed using the introduced operations, while still keeping the representation as small as possible. These operations form the basic building blocks for the further algorithms.
